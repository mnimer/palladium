        	...
        	<cftry>
                ...
                <cftry>
                    <cfoutput query="cfdebug_templates_summary">
                        <cfset templateOutput = encodeForError(template)>
                        <cfset templateAverageTime = Round(encodeForError(totalExecutionTime) / instances)>
                        ...
                        <cfif template EQ ExpandPath(cgi.script_name)>
                            <cfset templateOutput = "<img src='#getpageContext().getRequest().getContextPath()#/CFIDE/debug/images/topdoc.gif' alt='top level' border='0'> " &
							 ...
							 <cfif templateAverageTime GT cfdebugger.settings.template_highlight_minimum>
                                <cfset templateOutput = "<font color='red'><span class='template_overage'>" & encodeForError(template) & "</span></font>">
                                <cfset templateAverageTime = "<font color='red'><span class='template_overage'>" & templateAverageTime & "</span></font>">
								<cfset totalTime = "<font color='red'><span class='template_overage'>" & encodeForError(totalExecutionTime) & "</span></font>">
                            </cfif>
                        <cfelse>
                            <cfif templateAverageTime GT cfdebugger.settings.template_highlight_minimum>
                                <cfset templateOutput = "<font color='red'><span class='template_overage'>" & encodeForError(template) & "</span></font>">
                        ...
                        <tr>
							<cfif isDefined("totalTime") and len(trim(totalTime))>
								...
								<cfset totalTime = "">
							<cfelse>
    	                        <td align="right" class="cfdebug" nowrap>#encodeForError(totalExecutionTime)# ms</td>
							</cfif>
                            <td align="right" class="cfdebug" nowrap>#templateAverageTime# ms</td>
                            <td align="center" class="cfdebug" nowrap>#encodeForError(instances)#</td>
                ...
                <cfoutput>
                <tr>
					<td align="right" class="cfdebug" nowrap><i>#encodeForError(time_other)# ms</i></td><td colspan=2>&nbsp;</td>
                    <td align="left" class="cfdebug"><i>STARTUP, PARSING, COMPILING, LOADING, &amp; SHUTDOWN</i></td>
				</tr>
                <tr>
					<td align="right" class="cfdebug" nowrap><i>#encodeForError(cfdebug_execution.executionTime)# ms</i></td><td colspan=2>&nbsp;</td>
...
<cfoutput>
	...
	<cfloop query="cfdebug_ex">
	    <div class="cfdebug">#TimeFormat(cfdebug_ex.timestamp, "HH:mm:ss.SSS")# - #cfdebug_ex.name# <cfif FindNoCase("Exception", cfdebug_ex.name) EQ 0>Exception</cfif> - in #encodeForError(cfdebug_ex.template)# : line #encodeForError(cfdebug_ex.line)#</div>
	    <cfif IsDefined("cfdebug_ex.message") AND Len(Trim(cfdebug_ex.message)) GT 0>
	    <pre>
	    #encodeForError(cfdebug_ex.message)#
	    </pre>
	    </cfif>
	</cfloop>
</cfoutput>
...
<code><b>#cfdebug_storedproc.name#</b> (Datasource=#cfdebug_storedproc.datasource#, Time=#Max(cfdebug_storedproc.executionTime, 0)#ms) in #cfdebug_storedproc.template# @ #TimeFormat(cfdebug_storedproc.timestamp, "HH:mm:ss.SSS")#</code><br />
    ...
    <tr>
        ...
        <td>
            ...
            <tr>
                <td>&nbsp;<code><cfif StructKeyExists(thisParam, "type")>#encodeForError(thisParam.type)#</cfif></code></td>
                <td>&nbsp;<code><cfif StructKeyExists(thisParam, "sqlType")>#encodeForError(thisParam.sqlType)#</cfif></code></td>
                <td>&nbsp;<code><cfif StructKeyExists(thisParam, "value")>#htmleditformat(thisParam.value)#</cfif></code></td>
                <td>&nbsp;<code><cfif StructKeyExists(thisParam, "variable")>#encodeForError(thisParam.variable)# = #CFDebugSerializable(thisParam.variable)#</cfif></code></td>
                <td>&nbsp;<code><cfif StructKeyExists(thisParam, "dbVarName")>#encodeForError(thisParam.dbVarName)#</cfif></code></td>
            </tr>
            </cfloop>
            </table>
        </td>
...
<cfloop query="cfdebug_timer">
    <cftry>
    	<img src='#getpageContext().getRequest().getContextPath()#/CFIDE/debug/images/#Replace(cfdebug_timer.priority, " ", "%20")#_16x16.gif' alt="#cfdebug_timer.priority# type">
		 [#val(cfdebug_timer.endTime) - val(cfdebug_timer.startTime)#ms] <i>#encodeForError(cfdebug_timer.message)#</i><br />
    	<cfcatch type="Any"></cfcatch>
    </cftry>
</cfloop>
</p>
