...
<cfset myFusebox.thisCircuit = "comment">
<cfset myFusebox.thisFuseaction = "remove">
<cfif NOT structKeyExists(ATTRIBUTES, 'commentID')>
<cfthrow type="validation.INVALID_ID" message="Invalid Comment" detail="You must specify a valid comment to remove.">
</cfif>
<!--- do action="delete" --->
<cftry>
<cfset myFusebox.enterStackFrame() >
<cfif isDefined("variables.commentID")><cfset myFusebox.stack["variables.commentID"] = variables.commentID ></cfif>
<cfset variables.commentID = "#ATTRIBUTES.commentID#" >
<cfset myFusebox.thisFuseaction = "delete">
<cfif not isDefined("ATTRIBUTES.commentID")><cfset ATTRIBUTES.commentID = "0" /></cfif>
<cfset REQUEST.commentService = myFusebox.getApplication().getApplicationData().servicefactory.getBean(beanName="commentService")/> 
<cfset REQUEST.comment = REQUEST.commentService.getCommentByID(commentID="#ATTRIBUTES.commentID#") >
<cfset REQUEST.commentService.removeComment(commentID="#REQUEST.comment.getCommentID()#") >
<cfif structKeyExists(myFusebox.stack,"variables.commentID")><cfset variables.commentID = myFusebox.stack["variables.commentID"] ></cfif>
<cfif structKeyExists(myFusebox.stack,"variables.commentID")><cfset variables.commentID = myFusebox.stack["variables.commentID"] ><cfelse><cfset structDelete(variables,"commentID")></cfif>
