...
<cfset myFusebox.thisCircuit = "category">
<cfset myFusebox.thisFuseaction = "editForm">
<cfif NOT structKeyExists(ATTRIBUTES, 'categoryID')>
...
<cfset ATTRIBUTES.id = "#ATTRIBUTES.categoryID#" >
<cfset myFusebox.thisFuseaction = "get">
<cfif not isDefined("ATTRIBUTES.id")><cfset ATTRIBUTES.id = "0" /></cfif>
<cfset REQUEST.categoryService = myFusebox.getApplication().getApplicationData().servicefactory.getBean(beanName="categoryService")/> 
<cfset REQUEST.category = REQUEST.categoryService.getCategoryByID(categoryID="#ATTRIBUTES.id#") >
...
<cfset myFusebox.thisFuseaction = "editForm">
<cfif REQUEST.category.isNull()>
<cfthrow type="validation.NO_RECORDS_FOUND" message="Category Not Found" detail="The specified category could not be found.">
</cfif>
<!--- do action="outputForm" --->
<cftry>
<cfset myFusebox.enterStackFrame() >
<cfif isDefined("XFA.submit")><cfset myFusebox.stack["XFA.submit"] = XFA.submit ></cfif>
<cfset XFA.submit = "category.edit" >
<cfif isDefined("ATTRIBUTES.category")><cfset myFusebox.stack["ATTRIBUTES.category"] = ATTRIBUTES.category ></cfif>
<cfset ATTRIBUTES.category = "#REQUEST.category.getCategory()#" >
<cfset myFusebox.thisFuseaction = "outputForm">
<cftry>
<cfsavecontent variable="REQUEST.content.body"><cfoutput><cfinclude template="../home/category/dsp_categoryForm.cfm"></cfoutput></cfsavecontent>
<cfcatch type="missingInclude"><cfif len(cfcatch.MissingFileName) gte 20 and right(cfcatch.MissingFileName,20) is "dsp_categoryForm.cfm">
<cfthrow type="fusebox.missingFuse" message="missing Fuse" detail="You tried to include a fuse dsp_categoryForm.cfm in circuit category which does not exist (from fuseaction category.outputForm).">
<cfelse><cfrethrow></cfif></cfcatch></cftry>
<cfif structKeyExists(myFusebox.stack,"ATTRIBUTES.category")><cfset ATTRIBUTES.category = myFusebox.stack["ATTRIBUTES.category"] ></cfif>
<cfif structKeyExists(myFusebox.stack,"ATTRIBUTES.category")><cfset ATTRIBUTES.category = myFusebox.stack["ATTRIBUTES.category"] ><cfelse><cfset structDelete(ATTRIBUTES,"category")></cfif>
